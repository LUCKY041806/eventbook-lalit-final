    ...
    <div className="space-y-10 max-w-7xl mx-auto px-4 py-8">
      ...
      <section className="text-center space-y-3">
        ...
        <div className="space-x-3">
          ...
          <Link
            href="/organizer"
            className="inline-block border border-indigo-600 text-indigo-300 px-4 py-2 rounded text-sm hover:bg-indigo-50/10"
          >
            Organizer panel
          </Link>
        </div>
      ...
      <section className="space-y-3">
        <h3 className="font-semibold text-lg">Featured shows</h3>
        {featured.length === 0 ? (
            ...
            <div className="flex gap-4 min-w-max">
              {featured.map((event) => (
                <Link
                  ...
                  className="w-80 flex-shrink-0 bg-slate-900 rounded-xl overflow-hidden shadow-lg border border-slate-800 hover:border-indigo-500 transition snap-start"
                >
                  <div
                  ...
                  />
                  <div className="p-4 flex gap-3 items-center -mt-10">
                    {event.performerImageUrl && (
                      <img
                        src={event.performerImageUrl}
                        alt={event.performerName}
                        className="h-16 w-16 rounded-full object-cover border-2 border-slate-900 bg-slate-900"
                      />
                    )}
      ...
      <section className="space-y-3">
        <h3 className="font-semibold text-lg">All upcoming events</h3>
        {regular.length === 0 ? (
          <p className="text-sm text-slate-500">
            ...
            <div className="flex gap-4 min-w-max">
              {regular.map((event) => (
                <Link
                  key={event._id}
                  href={`/events/${event._id}`}
                  className="w-80 flex-shrink-0 border border-slate-800 rounded-xl bg-slate-950/60 hover:border-indigo-500 transition-all duration-300 overflow-hidden snap-start"
                >
                  <div
                    className="h-64 bg-cover bg-center"
                    style={{
